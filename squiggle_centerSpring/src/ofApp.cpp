...
void squiggle::makeSquiggleShape(int width, int height) {
    ofPolyline squiggle;
    ofPoint origin = ofPoint(ofGetWidth() / 2, ofGetHeight() * 3/4); // TODO: change this to be parameter
    ofPoint currPos;
...
void squiggle::makeInternalSpring(int width, int height) {
    ofPoint origin = ofPoint(ofGetWidth() / 2, ofGetHeight() * 3/4);
    float resolution = 100;
    float stepSize = height/resolution;
    for(float i = 0; i < height; i += stepSize) {
        particle * p = new particle();
        ofPoint pos = origin+ofPoint(0,i);
        p->setInitialCondition(pos.x,pos.y,pos.z, 0, 0, 0);
        particles.push_back(p);
    }
...
//            particle->addForce(move.x,move.y,move.z);
//        }
        for(int i = 1; i < particles.size(); i++) {
            particles[i]->addForce(move.x,move.y,move.z);
        }
        particles[0]->pos += move;
    }
